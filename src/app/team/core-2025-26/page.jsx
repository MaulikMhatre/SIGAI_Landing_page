...
/**
 * Reusable 3D Gradient Member Card - Optimized for Mobile & Desktop
 */
  ...
  return (
    <motion.div
      ...
      transition={{ 
        y: isActive 
          ...
          : { duration: 4, repeat: Infinity, ease: "easeInOut", delay: delay * 0.2 },
        rotateX: isActive 
          ? { type: "spring", stiffness: 300, damping: 20 } 
          : { duration: 6, repeat: Infinity, ease: "easeInOut", delay: delay * 0.1 },
        rotateY: isActive 
          ? { type: "spring", stiffness: 300, damping: 20 } 
          : { duration: 6, repeat: Infinity, ease: "easeInOut", delay: delay * 0.3 },
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: `radial-gradient(circle at center, rgba(103, 198, 254, 0.2) 0%, transparent 80%)`,
          filter: "blur(40px)",
        }}
        animate={{
          x: isActive ? rotation.y * 5 : 0,
          y: isActive ? -rotation.x * 5 : 0,
        }}
      />
      ...
      <div className="relative h-full flex flex-col items-center justify-between" style={{ transform: "translateZ(30px)" }}>
        ...
        <div className="relative w-full h-64 overflow-hidden rounded-xl bg-black/20 shadow-inner">
          <Image
            ...
            fill
            sizes="256px"
            priority={delay < 0.5} // Loads first row instantly
          ...
          <div className="hidden md:flex absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 items-center justify-center gap-6">
            <a href={person.instagram} target="_blank" rel="noopener noreferrer" className="text-white text-3xl hover:text-[#ff4f84] transition-transform hover:scale-125">
              <FontAwesomeIcon icon={faInstagram} />
            </a>
            <a href={person.linkedin} target="_blank" rel="noopener noreferrer" className="text-white text-3xl hover:text-[#4fa1ff] transition-transform hover:scale-125">
              <FontAwesomeIcon icon={faLinkedin} />
            </a>
          </div>
        </div>

        {/* Info & Mobile persistent icons */}
        <div className="text-center w-full mt-3" style={{ transform: isMounted ? "translateZ(40px)" : "none" }}>
          ...
          <div className="flex md:hidden items-center justify-center gap-6 mt-3 pt-2 border-t border-white/5">
            <a href={person.instagram} target="_blank" rel="noopener noreferrer" className="text-gray-400 active:text-[#ff4f84] text-xl transition-colors">
              <FontAwesomeIcon icon={faInstagram} />
            </a>
            <a href={person.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-400 active:text-[#4fa1ff] text-xl transition-colors">
              <FontAwesomeIcon icon={faLinkedin} />
            </a>
          </div>
        </div>
      ...
      <motion.div 
        className="absolute bottom-0 left-4 right-4 h-[2px] bg-gradient-to-r from-transparent via-[#64c3fa] to-transparent"
...
const Core2025_26 = () => {
  const faculty = [
    { name: "Dr. Aruna Gawade", role: "Faculty Coordinator", img: "/team/2025-26/aruna-mam.png", instagram: "https://instagram.com", linkedin: "https://linkedin.com" },
    { name: "Prof. Ragini Mishra", role: "Faculty Coordinator", img: "/team/2025-26/ragini-mam.png", instagram: "https://instagram.com", linkedin: "https://linkedin.com" },
  ];
  ...
  return (
    <div className="grad-bg min-h-screen text-white flex flex-col items-center pt-24 pb-20 relative overflow-x-hidden">
      <AIBackground />

      <div className="relative z-10 flex flex-col items-center w-full px-4">
        <h2 className="text-4xl text-center primary-font font-extrabold mb-12 tracking-tight uppercase">
        ...
        <div className="flex flex-wrap justify-center gap-10 md:gap-16 mb-24">
          {faculty.map((person, index) => (
            <MemberCard key={index} person={person} delay={index * 0.15} />
          ))}
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-10 md:gap-12 mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10 md:gap-12">
          {core.slice(2).map((person, index) => (
            <MemberCard key={index + 2} person={person} delay={(index % 4) * 0.1} />
          ))}
        </div>
      </div>
    </div>
